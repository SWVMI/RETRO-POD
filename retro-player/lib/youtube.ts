...
async function initializeWebPoMinter(): Promise<WebPoMinter> {
  ...
  // Keep the browser options separate so TypeScript does not apply an
  // excess-property check to jsdom's version-dependent ConstructorOptions.
  const domOptions = {
  ...
  Object.defineProperty(globalThis, "origin", { configurable: true, value: dom.window.origin });
  ...
  try {
    ...
    if (!pageResponse.ok) {
      throw new Error(`YouTube homepage returned HTTP ${pageResponse.status}.`);
    }
    const pageHtml = await pageResponse.text();
    const ytConfigMatch = pageHtml.match(/ytcfg\.set\(({.+?})\);/s);
    if (!ytConfigMatch) throw new Error("Could not find YouTube ytcfg.");
    const ytConfig = JSON.parse(ytConfigMatch[1]);
    (dom.window as any).yt = { config_: ytConfig };
    ...
    if (!challengeResponse?.bgChallenge) throw new Error("YouTube did not provide a BotGuard challenge.");

    const interpreterUrl = challengeResponse.bgChallenge.interpreterUrl
      ?.privateDoNotAccessOrElseTrustedResourceUrlWrappedValue;
    if (!interpreterUrl) throw new Error("YouTube did not provide a BotGuard interpreter URL.");
    ...
    if (!bgScriptResponse.ok) {
      throw new Error(`BotGuard interpreter returned HTTP ${bgScriptResponse.status}.`);
    }
    const interpreterJavascript = await bgScriptResponse.text();
    if (!interpreterJavascript) throw new Error("YouTube returned an empty BotGuard interpreter.");
    ...
    if (!integrityTokenResponse.ok) {
      throw new Error(`GenerateIT returned HTTP ${integrityTokenResponse.status}.`);
    }
...
async function getWebPoMinter(): Promise<WebPoMinter> {
  ...
  if (!webPoInitPromise) webPoInitPromise = initializeWebPoMinter();
  try {
...
async function getWebPoToken(videoId: string): Promise<string> {
  ...
  if (!webPoDom) throw new Error("WebPO browser environment is not available.");
  const token = await minter.mintAsWebsafeString(videoId);
  if (!token) throw new Error(`Failed to mint WebPO token for ${videoId}.`);
...
function thumbnailFrom(item: any): string {
  const thumbnails = item?.thumbnails || item?.thumbnail?.thumbnails || item?.video_thumbnails || [];
  return thumbnails[thumbnails.length - 1]?.url || thumbnails[0]?.url || "";
}

function durationFrom(item: any): number {
  if (typeof item?.duration === "number") return item.duration;
  if (typeof item?.duration?.seconds === "number") return item.duration.seconds;
...
function mapVideo(item: any): YouTubeTrack | null {
  const id = item?.id || item?.video_id || item?.endpoint?.payload?.videoId;
  if (!id) return null;
  return {
    id,
    title: item?.title?.text || item?.title || "Unknown",
    artist: item?.author?.name || item?.author?.text || item?.artists?.[0]?.name || "YouTube",
...
export async function createYouTubeAudioStream(videoId: string) {
  const yt = await getYouTube();
  console.log(`[YouTube Audio] Resolving audio-only stream for ${videoId}`);
  try {
    console.log(`[YouTube Audio] Trying MWEB client for ${videoId}`);
    ...
    if (!url) throw new Error("YouTube returned no playable MWEB audio URL.");
    console.log(`[YouTube Audio] MWEB selected itag=${format.itag}, mime=${format.mime_type}`);
    const poToken = await getWebPoToken(videoId);
    const separator = url.includes("?") ? "&" : "?";
    const playableUrl = `${url}${separator}pot=${encodeURIComponent(poToken)}`;
    console.log(`[YouTube Audio] WebPO token attached to ${videoId}`);
    return {
      url: playableUrl,
      mimeType: format.mime_type || "audio/mp4",
      duration: format.approx_duration_ms ? Number(format.approx_duration_ms) / 1000 : undefined,
    };
  } catch (error) {
    console.error(`[YouTube Audio] MWEB + WebPO failed for ${videoId}:`, error);
    throw new Error(`Unable to resolve a playable YouTube audio stream for ${videoId}. ${error instanceof Error ? error.message : String(error)}`);
  }
}
